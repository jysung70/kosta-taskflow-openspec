...
### Requirement: 태스크 생성
시스템은 팀 소속 멤버가 태스크를 생성할 수 있어야 한다(SHALL). 초기 상태는 항상 TODO(MUST). creator_id는 현재 로그인 사용자.

#### Scenario: 정상 생성
- **WHEN** POST /api/teams/1/tasks {"title": "로그인 화면 구현"} (팀 멤버 로그인 상태)
- **THEN** HTTP 201, {"id": 1, "title": "로그인 화면 구현", "status": "TODO", "creator_id": 1, "team_id": 1}
